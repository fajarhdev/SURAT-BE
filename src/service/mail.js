...
const createOutMailService = async (mail, user) => {
	...
	try {
		...
		const numCodeMail = `${codeMail.code} ${numMail}/${problem.code}/${executive.code}/REG3/${currentYear}`;
...
const updateOutMailService = async (mail, user, id) => {
	...
	try {
		...
		const numCodeMail = `${codeMail.code} ${numCode}/${problem.code}/${executive.code}/REG3/${currentYear}`;
