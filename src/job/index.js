...
async function startPgBoss() {
	...
	// Schedule the jobs
	await boss.schedule("nomor-surat-cadangan-update", "0 0 * * 5", {
		priority: 2,
		timezone: "Asia/Jakarta",
	}); // every Friday
	await boss.schedule("update-tanggal", "0 0 * * *", {
		priority: 1,
		timezone: "Asia/Jakarta",
	}); // every day 00:00
	await boss.schedule("reset-nomor-surat", "0 0 1 1 *", {
		priority: 2,
		timezone: "Asia/Jakarta",
	}); // every year
	await boss.schedule("delete-photo", "0 0 1 */6 *", {
		priority: 3,
		timezone: "Asia/Jakarta",
	}); // every 6 months
